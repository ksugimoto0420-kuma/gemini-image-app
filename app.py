...
def main():
    ...
    if mode == "📝 テキストから生成":
        ...
        # 画像生成のコツをアコーディオンで表示
        with st.expander("💡 画像生成のコツ・プロンプトテンプレート"):
            st.markdown("""
# Gemini 3 Flash & Nano Banana Pro 利用ガイド

作成日: 2025年12月18日

---

## 1. Gemini 3 Flash について

**概要**
2025年12月17日にリリースされた最新モデルです。「高速・低コスト」でありながら、前世代のProモデルに匹敵する知能を持っています。

**主な進化点（何が変わったのか）**
* **速度:** 前世代のGemini 2.5 Proと比較して約3倍高速。
* **効率性:** 「思考プロセス」が最適化され、複雑なタスクも少ない計算量で処理可能。
* **コスト:** 非常に安価な価格設定（入力 $0.50 / 出力 $3.00 per 1M tokens）。

**何ができるのか**
* **高度なマルチモーダル処理:** テキスト、画像、音声、動画を同時に理解。
* **思考モードの利用:** 論理パズルやコーディングなどの複雑な問いに対して、AIの思考過程を確認可能。
* **画像生成:** **Gemini 3 Flash自体は非対応**（画像生成は下記のNano Banana Proが担当）。

---

## 2. Nano Banana Pro (Gemini 3 Pro Image) について

**概要**
2025年11月20日にリリースされた、Googleの最高品質画像生成モデルです。

**主な特徴**
* **文字再現性:** 画像内の文字（日本語含む）の描写能力が飛躍的に向上。
* **一貫性:** キャラクターや被写体の一貫性を保つ能力が強化されており、着せ替えや合成が得意。
* **用途:** ビジネス資料作成からクリエイティブな編集まで幅広く対応。

---

## 3. 画像合成・着せ替えプロンプトテンプレート集

Nano Banana Proを使用して、既存の画像（写メなど）を編集する際に有効なプロンプトです。

### 基本構造
1. **参照指示:** 元画像を使うことを明言する。
2. **維持する要素:** 顔、体型、ポーズ、背景など。
3. **変更する要素:** 具体的な服装、アイテム、場所。

### テンプレート

#### 【テンプレート1】人物：ポーズと背景を維持した着せ替え
```
参照画像の人物に基づき、現在着用している服を[新しい服装の具体的な説明、例：ネイビーのテーラードジャケットと白いリネンシャツ]に変更してください。

以下の要素は元の画像と完全に同じ状態を維持してください：
* 人物の顔立ち、髪型、表情
* 現在のポーズ
* 背景の[背景の簡単な説明、例：カフェの店内]と照明の雰囲気

新しい服の素材感がリアルに見える写真として生成してください。
```

#### 【テンプレート2】動物：ペットに衣装・アイテムを追加
```
参照画像の[動物の種類と特徴、例：茶トラの猫]が、新しく[追加したいアイテム、例：赤いサンタクロースの帽子と小さなケープ]を身に着けている様子を生成してください。

動物の元の姿勢、視線、毛並みの質感はそのまま維持してください。衣装は体に自然にフィットしており、違和感がないようにしてください。背景は変更しないでください。
```

#### 【テンプレート3】応用：服装と背景を両方変更（顔は維持）
```
参照画像の人物をモデルにした、新しいポートレート写真を作成してください。

* 人物: 参照画像の人物の顔立ちと髪型を維持する。
* 新しい服装: [具体的な服装、例：黒いイブニングドレスとシルバーのネックレス]を着用している。
* 新しい場所: [新しい背景、例：夜景が見える高級ホテルのバルコニー]に立っている。
* 雰囲気: 夜の落ち着いた照明で、背景の街明かりがボケている、高品質な写真。
```

#### 【テンプレート4】部分変更：髪型やメイクのみ変更
```
参照画像の人物について、服装と背景はそのまま維持し、髪型とメイクだけを変更してください。

* 新しい髪型: [具体的な髪型、例：肩までの長さのウェーブがかかったボブ、色は明るいブラウン]
* 新しいメイク: [具体的なメイク、例：ナチュラルなアイメイクと赤いリップ]

元の画像の自然な写真の質感を保ってください。
```

### 成功のためのコツ
1. **「維持する」指示を徹底する:** 「顔は変えないで」「ポーズはそのままで」としつこいくらいに明記する。
2. **素材感を伝える:** 「デニム」「シルク」「ウール」など、服の素材を指定するとリアリティが増す。
3. **ライティングを合わせる:** 「元の画像と同じ自然光で」などの指示を加えると合成感が減る。
            """)
        ...
        # 画像生成のコツをアコーディオンで表示
        with st.expander("💡 画像加工・合成のコツ・プロンプトテンプレート"):
            st.markdown("""
# 画像合成・着せ替えプロンプトテンプレート集

Nano Banana Pro (Gemini 3 Pro Image) を使用して、既存の画像（写メなど）を編集する際に有効なプロンプトです。
